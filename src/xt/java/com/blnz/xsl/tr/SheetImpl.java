// $Id: SheetImpl.java 113 2005-03-28 21:52:49Z blindsey $
...
import com.blnz.xsl.expr.ExprParser;
...
import com.blnz.xsl.expr.NumberVariant;
...
import com.blnz.xsl.util.TextComparator;
import com.blnz.xsl.util.NumberComparator;
import com.blnz.xsl.util.ReverseComparator;
    ...
    implements SheetDetails, LoadContext, NamespaceConstants
{
    ...
    private ImportantBoolean _stripSource = null;

    private Hashtable _stripSourceElementTable = null;

    private Hashtable _stripSourceNamespaceTable = null;

    private TemplateRuleSet _templateRules = 
        new TemplateRuleSet(new BuiltinAction());

    private Hashtable _modeTable = new Hashtable();

    private Hashtable _namedTemplateTable = new Hashtable();

    private Hashtable _variableInfoTable = new Hashtable();
    ...
    private Hashtable v2TopLevelTable = new Hashtable();

    private Hashtable _actionTable = new Hashtable();

    private Hashtable _v2ActionTable = new Hashtable();
    ...
    int _nCurrentLocalVariables = 0;

    // xslt v2 stylesheet?
    private boolean _ssV2 = false;

    Vector excludedNamespaces;

    XMLProcessor _omBuilder;

    static final StringVariant emptyStringVariant = new StringVariant("");
	...
	VERSION = nameTable.createName("version");
        ...
        v2TopLevelTable.put(xsl("character-map"), new CharacterMapTopLevelParser());
        v2TopLevelTable.put(xsl("function"), new FunctionTopLevelParser());
        v2TopLevelTable.put(xsl("import-schema"), new ImportSchemaTopLevelParser());
	...
	_actionTable.put(xsl("text"), new TextParser());
	_actionTable.put(xsl("apply-templates"), new ApplyTemplatesParser());
	_actionTable.put(xsl("call-template"), new CallTemplateParser());
	_actionTable.put(xsl("for-each"), new ForEachParser());
	_actionTable.put(xsl("value-of"), new ValueOfParser());
	_actionTable.put(xsl("number"), new NumberParser());
	_actionTable.put(xsl("if"), new IfParser());
	_actionTable.put(xsl("choose"), new ChooseParser());
	_actionTable.put(xsl("copy"), new CopyParser());
	_actionTable.put(xsl("copy-of"), new CopyOfParser());
	_actionTable.put(xsl("comment"), new CommentParser());
	_actionTable.put(xsl("processing-instruction"), 
                        new ProcessingInstructionParser());
	_actionTable.put(xsl("element"), new ElementParser());
	_actionTable.put(xsl("attribute"), new AttributeParser());
	_actionTable.put(xsl("apply-imports"), new ApplyImportsParser());
	_actionTable.put(xsl("variable"), new VariableActionParser());
	_actionTable.put(xsl("param"), new ParamActionParser());
	_actionTable.put(xsl("message"), new MessageParser());

	_actionTable.put(xt("document"), new DocumentParser());

        _actionTable.put(exsl("document"), new DocumentParser());

        _actionTable.put(func("result"), new EXSLFunctionResultParser());

	_actionTable.put(xfyxt("xrap"), new XRAPParser());
	...
	_templateRules.compile();

	for (Enumeration iter = _modeTable.elements() ; 
    ...
    void parseTopLevel(Node sheetNode) throws XSLException, IOException
    {
        ...
        //   System.out.println("parseTopLevel:: {" + sheetNode.getName().getNamespace() + 
        //                      "}" +  sheetNode.getName().getLocalPart());


        // note if stylesheet is "1.0" or "2.0"
        _ssV2 = "2.0".equals(getRequiredAttribute(sheetNode, VERSION));
	...
	for (NodeIterator iter = sheetNode.getChildren();;) {
		...
		if (name == null) {
		    throw new XSLException("illegal data characters inside xsl:stylesheet", node);
                ...
                if (_ssV2) {
                    parser = (TopLevelParser)v2TopLevelTable.get(name);
                }
                if (parser == null) {
                    parser = (TopLevelParser)topLevelTable.get(name);
                }
		if (parser == null && name.getNamespace() == null) {
		    throw new XSLException("illegal top-level element with no namespace", node);
                }
	    }
    ...
    private void parseRootTemplate(Node defNode) throws XSLException
    {
        ...
        // get the root Element
	Node sheetNode = defNode.getChildren().next();  
        ...
        // FIXME:  should be required attr
        _ssV2 = "2.0".equals(getOptionalAttribute(sheetNode, XSL_VERSION, "1.0"));


	_templateRules.add(ExprParser.parsePattern(defNode, "/"),
			  currentImportance,
			  null,
    ...
    Action parseAttributesAndActions(Node node) throws XSLException
    {
        ...
        //        System.out.println("parseAttributesAndActions:: {" + node.getName().getNamespace() + 
	...
	for (NodeIterator iter = node.getAttributes();;) {
		...
		sequence.add(new TemplateAttributeAction(name,
                                                         _ssV2 ? 
							 Expr2Parser.parseValueExpr(node, value, currentLocalVariables) :
    ...
    {
        //        System.out.println("parseActions1:: {" + node.getName().getNamespace() + 
        //                           "}" +  node.getName().getLocalPart());
    ...
    Action parseActions(Node node, Action ifEmpty, 
                        AppendAction sequence) throws XSLException
    {
        //        System.out.println("parseActions2:: {" + node.getName().getNamespace() + 
        //                           "}" +  node.getName().getLocalPart());

	final VariableSet startLocalVariables = currentLocalVariables;
	final int nStartLocalVariables = _nCurrentLocalVariables;
	...
	do {
	    ...
	    case Node.ELEMENT:
		{
			...
			try {
			    actionParser = (ActionParser) _actionTable.get(name);
			} catch (ClassCastException e) { 
	...
	if (nStartLocalVariables != _nCurrentLocalVariables) {
	    sequence.add(new UnbindLocalVariablesAction(_nCurrentLocalVariables -
                                                        nStartLocalVariables));
	    _nCurrentLocalVariables = nStartLocalVariables;
	...
	for (NodeIterator iter = node.getChildren();;) {
		...
		ComparatorTemplate templ 
		    = new NodeComparatorTemplate(cmp,
                                                 _ssV2 ?
						 Expr2Parser.parseStringExpr(node,
	...
	return _ssV2 ? 
            Expr2Parser.parseVariantExpr(defNode, expr, currentLocalVariables) :
            ExprParser.parseVariantExpr(defNode, expr, currentLocalVariables);
    }
	...
	if (modeName == null) {
	    ...
	    return _templateRules;
        }
	TemplateRuleSet ruleSet = (TemplateRuleSet) _modeTable.get(modeName);
	if (ruleSet == null) {
	    ruleSet = new TemplateRuleSet(new BuiltinAction(modeName));
	    _modeTable.put(modeName, ruleSet);
	}
	return ruleSet;
    }
    ...
    {
	ImportantBoolean match = _stripSource;
	if (_stripSourceNamespaceTable != null) {
	    String ns = elementTypeName.getNamespace();
	    if (ns != null) {
		ImportantBoolean ib = 
                    (ImportantBoolean) _stripSourceNamespaceTable.get(ns);
		if (ib != null && 
                    (match == null || 
	...
	if (_stripSourceElementTable != null) {
	    ImportantBoolean ib = 
                (ImportantBoolean) _stripSourceElementTable.get(elementTypeName);
	    if (ib != null && 
                (match == null || 
                 ib.getImportance().compareTo(match.getImportance()) >= 0)) {
		match = ib;
    ...
    {
	return (VariableInfo) _variableInfoTable.get(name);
    }
	...
	if (name.equals(XSL_VENDOR)) {
	    return new StringVariant("Bill Lindsey");
        }
	if (name.equals(XSL_VENDOR_URL)) {
	    return new StringVariant("http://www.blnz.com/");
        }
    ...
    private class TemplateParser implements TopLevelParser
    {
        public void parse(Node defNode) throws XSLException
        {
            ...
            if (name != null) {
                _namedTemplateTable.put(expandSourceElementTypeName(name, 
                                                                   defNode),
                                       contents);
            }
            ...
            if (modeString != null) {
                ...
                ruleSet = _templateRules;
            }
            try {
                ruleSet.add(_ssV2 ? 
                            Expr2Parser.parsePattern(defNode, pattern) :
                            ExprParser.parsePattern(defNode, pattern),
                            currentImportance,
    ...
    private class KeyParser implements TopLevelParser
    {
	    ...
	    TopLevelPattern matchPattern = _ssV2 ?
                Expr2Parser.parsePattern(defNode, pattern) :
                ExprParser.parsePattern(defNode, pattern);
	    ...
	    StringExpr useExpr = 
                _ssV2 ?
		Expr2Parser.parseStringExpr(defNode, use, currentLocalVariables) :
    ...
    // XSLT version 2.0  "xsl:character-map"
    private class CharacterMapTopLevelParser implements TopLevelParser
    {
    ...
    // XSLT version 2.0  "xsl:function"
    private class FunctionTopLevelParser implements TopLevelParser
    {

        public void parse(Node defNode) throws XSLException
        {

            Name functionName = expandSourceElementTypeName(getRequiredAttribute(defNode,
                                                                                 NAME),
                                                            defNode);
    ...
    // XSLT version 2.0 "xsl:import-schema"
    private class ImportSchemaTopLevelParser implements TopLevelParser
    {
    ...
    private class VariableTopLevelParser implements TopLevelParser
    {
        public void parse(Node defNode) throws XSLException
        {
            //     System.out.println("VariableTopLevelParser:parse1(): {" + 
            //                        defNode.getName().getNamespace() + "}" +  defNode.getName().getLocalPart());
            parse(defNode, false);
        }
        ...
        void parse(Node defNode, boolean isParam) throws XSLException
        {
            ...
            //            System.out.println("VariableTopLevelParser:parse2(): {" +
            //                                defNode.getName().getNamespace() + "}" +  defNode.getName().getLocalPart());
            _variableInfoTable
            ...
            while (iter.hasMoreElements()) {
                ...
                if (name.getLocalPart().equals("*")) {
                    ...
                    if (ns == null) {
                        ...
                        if (_stripSourceNamespaceTable == null) {
                            _stripSourceNamespaceTable = new Hashtable();
                        }
                        _stripSourceNamespaceTable.put(ns, ib);
                    }
                }
                else {
                    if (_stripSourceElementTable == null) {
                        _stripSourceElementTable = new Hashtable();
                    }
                    _stripSourceElementTable.put(name, ib);
                }
            }
        }
    }
    ...
    private class ApplyTemplatesParser implements ActionParser
    {
        ...
        NodeSetExpr getNodeSetExpr(Node node) throws XSLException
        {
            ...
            return 
                _ssV2 ?
                Expr2Parser.parseNodeSetExpr(node, select, currentLocalVariables) :
                ExprParser.parseNodeSetExpr(node, select, currentLocalVariables);
        }
    ...
    private class ForEachParser implements ActionParser
    {
        public Action parse(Node node) throws XSLException
        {
            ...
            NodeSetExpr select = 
                _ssV2 ? 
                Expr2Parser.parseNodeSetExpr(node, 
                                             getRequiredAttribute(node, SELECT), 
                                             currentLocalVariables) :
                ExprParser.parseNodeSetExpr(node, 
                                             getRequiredAttribute(node, SELECT), 
                                             currentLocalVariables);
                                 
            if (_debugger != null) {
                return new ForEachActionDebug(_debugger, node, "",
                                              getSortNodeSetExpr(node, select),
                                              parseActions(node, emptyAction));
            } else {
                return new ForEachAction(getSortNodeSetExpr(node, select),
                                         parseActions(node, emptyAction));
            }
        }
    }
    ...
    private class IfParser implements ActionParser
    {
        ...
        {
            return _ssV2 ? 
                Expr2Parser.parseBooleanExpr(node,
                                             getRequiredAttribute(node, TEST),
                                             currentLocalVariables) :
                ExprParser.parseBooleanExpr(node,
                                            getRequiredAttribute(node, TEST),
                                            currentLocalVariables);
        }
    }
    ...
    private class ProcessingInstructionParser implements ActionParser
    {
        public Action parse(Node node) throws XSLException
        {
            if (_debugger != null) {
                ...
                return new
                    ProcessingInstructionAction(_ssV2 ?
                                                Expr2Parser.
                                                parseValueExpr(node,
                                                               getRequiredAttribute(node, NAME),
                                                               currentLocalVariables) :
    ...
    private class ElementParser implements ActionParser
    {
        ...
        public Action parse(Node node) throws XSLException
        {
            if (_debugger != null) {
                ...
                return new ElementAction(_ssV2 ?
                                         ExprParser.
                                         parseValueExpr(node,
                                                        getRequiredAttribute(node, 
                                                                             NAME),
        ...
        StringExpr getNamespaceExpr(Node node) throws XSLException
        {
            ...
            return _ssV2 ? 
                ExprParser.parseValueExpr(node, 
                                          namespace, 
                                          currentLocalVariables) :
                ExprParser.parseValueExpr(node, 
                                          namespace, 
    ...
    private class AttributeParser extends ElementParser
    {
        ...
        public Action parse(Node node) throws XSLException
        {
            ...
            if (_debugger != null) {
                ...
                return new AttributeAction(nameExpression,
                                           getNamespaceExpr(node),
                                           node.getNamespacePrefixMap(),
                                           parseActions(node, emptyAction));
            }
    ...
    private class DocumentParser implements ActionParser
    {
        ...
        public Action parse(Node node) throws XSLException
        {
            return new DocumentAction(_ssV2 ?
                                      Expr2Parser.
                                      parseValueExpr(node,
                                                     getRequiredAttribute(node,
                                                                          HREF),
    ...
    private class NumberParser implements ActionParser
    {
        public Action parse(Node node) throws XSLException
        {
            ...
            if (countString == null) {
                ...
                count = 
                    _ssV2 ?
                    Expr2Parser.parsePattern(node, countString, currentLocalVariables) :
                    ExprParser.parsePattern(node, countString, currentLocalVariables);
	    }
            ...
            if (fromString == null) {
                ...
                from = 
                    _ssV2 ?
                    Expr2Parser.parsePattern(node, fromString, currentLocalVariables) :
                    ExprParser.parsePattern(node, fromString, currentLocalVariables);
            }
            if (level.equals("any")) {
                if (_debugger != null) {
