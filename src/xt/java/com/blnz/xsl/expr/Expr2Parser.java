 ...
 */
public class Expr2Parser extends Expr2Tokenizer 
    implements NamespaceConstants 
{
    ...
    public static TopLevelPattern parsePattern(Node node, 
        ...
        throws XSLException 
    {
        return new Expr2Parser(pattern, node, new EmptyVariableSet())
    ...
    public static TopLevelPattern parsePattern(Node node, 
        ...
        throws XSLException 
    {
        return new Expr2Parser(pattern, node, locals)
            .parseTopLevelPattern(node);
    }
    ...
    {
        return new Expr2Parser(expr, node, locals).parseExpr();
    }
    ...
    //
    //
    private Expr2Parser(String expr, Node node, VariableSet locals) 
    {
        super(expr);
        this.node = node;
        if (node != null) {
    ...
    {
        next();
        //        System.out.println("parseExpr: value is " + _currentTokenValue);
        ConvertibleExpr expr = parseSeqExpr();
    ...
    //
    //
    private ConvertibleExpr parseSeqExpr() throws ParseException 
    {

        ConvertibleExpr expr = parseExprSingle();
    ...
    private ConvertibleExpr parseExprSingle()
        throws ParseException 
    {
    ...
    // 
    //
    private ConvertibleExpr parseIfExpr() throws ParseException 
    {

        try {
            // System.out.println("parseIfExpr");
            // current token is "if("
            
            //            System.out.println("next()._currentTokenValue: " + _currentTokenValue);

            next() ;
            BooleanExpr testExpr = parseExprSingle().makeBooleanExpr();
            
            expectRpar();
            
            //            System.out.println("next()._currentTokenValue (want 'then') : " + _currentTokenValue);

            if (currentToken != TOK_THEN) {
                throw new ParseException("expected 'then' in if expression");
            }
            // current should be "then"
            next();
            VariantExpr thenExpr = parseExprSingle().makeVariantExpr();
            
            //         System.out.println("next()._currentTokenValue should be 'else': " + _currentTokenValue);
            //        currrent should be "else"

            if (currentToken != TOK_ELSE) {
                throw new ParseException("expected 'else' in if expression");
            }

            next();
            VariantExpr elseExpr = parseExprSingle().makeVariantExpr();
    ...
    private ConvertibleNodeSetExpr parseStep() throws ParseException 
    {
        ...
        case TOK_AXIS:
            {
                AxisExpr axis = (AxisExpr)axisTable.get(_currentTokenValue);
                if (axis == null) {
                    throw new ParseException("no such axis");
		}
                boolean isAttribute = _currentTokenValue.equals("attribute");
                next();
    ...
    private ConvertibleExpr parsePrimaryExpr() throws ParseException 
    {
        ...
        case TOK_LITERAL:
            ...
            expr = new LiteralExpr(_currentTokenValue);
            break;

        case TOK_NUMBER:
            // prod #30 (handled by lexer)
            expr = new NumberConstantExpr(Converter.toNumber(_currentTokenValue));
            break;
		...
		// a CallExpression  
                Function function = 
                    (Function) functionTable.get(_currentTokenValue);

                if (function == null) {
                    // "current()" is special, 'cuz we'll want
                    // to take note of the fact that this expr
                    // uses it
                    if (!_currentTokenValue.equals("current")) {
    ...
    private ConvertibleExpr[] parseArgs() throws ParseException 
    {
        ...
        for (;;) {
            args[args.length - 1] = parseExprSingle();
            if (currentToken != TOK_COMMA) {
                break;
            }
            next();
    ...
    private Name expandName() throws ParseException 
    {
        try {
            if (prefixMap != null) {
                return prefixMap.expandAttributeName(_currentTokenValue, null);
            } else {
                return null;
            }
    ...
    private String expandPrefix() throws ParseException 
    {
        ...
        String ns = prefixMap.getNamespace(_currentTokenValue);
        if (ns == null)
            throw new ParseException("undefined prefix");
        return ns;
    }
    ...
    //  
    // return the index of the end of the XPath expression 
        ...
        throws ParseException 
    {
        ...
        case TOK_FUNCTION_LPAR:
            if (_currentTokenValue.equals("id")) {
                next();
                if (currentToken != TOK_LITERAL) {
                    throw new ParseException("expected literal");
		}
                PathPatternBase tem = new IdPattern(_currentTokenValue);
                next();
                expectRpar();
                if (currentToken == TOK_SLASH)
                    parent = tem;
                ...
                next();
                break;
            } else if (_currentTokenValue.equals("key")) {

                next();
                if (currentToken != TOK_LITERAL) {
                    throw new ParseException("key pattern: expected literal arg 1");
		}
		String keyName = _currentTokenValue;
		next();
		...
		next();
                ...
                PathPatternBase tem = new KeyPattern(keyName, _currentTokenValue);
                next();
                expectRpar();
                if (currentToken == TOK_SLASH) {
                    parent = tem;
    ...
    private PathPatternBase parseStepPattern() throws ParseException 
    {
        ...
        if (currentToken == TOK_AT
            || (currentToken == TOK_AXIS
                && _currentTokenValue.equals("attribute"))) {
            next();
            ...
            if (currentToken == TOK_AXIS
                && _currentTokenValue.equals("child")) {
                next();
            }
